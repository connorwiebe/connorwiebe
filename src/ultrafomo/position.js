...
export default function Position({ symbol, position, setPositions }) {
  ...
  return (
    <div className="stat-item">
      <div data-name="Symbol" className="stat-data symbol">
        <div
          className="stat-data-color"
          style={{ background: position.color }}
        />
        <span className="stat-data-symbol">{symbol}</span>
      </div>
      <span data-name="Profit" className="stat-data">
        {position.formatted.profit}
      </span>
      <span data-name="Dividends" className="stat-data">
        {position.formatted.dividends}
      </span>
      <span data-name="Return" className="stat-data">
        {position.formatted.roi}
      </span>
      <div data-name="Allocation" className="stat-data percentage">
        <select
          onChange={changePercentage}
          defaultValue={position.percentage}
          name="percentage"
          id={symbol}
        >
      ...
      <span data-name="Annualized Return" className="stat-data">
        {position.formatted.annualized}
      </span>
      <button
        onClick={deletePosition}
        id={symbol}
        className={`delete ${loading ? 'loading-dark' : ''}`}
      >
        <svg
